...
im = 1 #Intervalo de medición para activar el if, es corto, para medir rápido al llegar a 85. 
    ...
    if R>85:
        if R<100:
            ...
            for i in range(N):
                ...
                df=pd.DataFrame(mediciones)
                #print(time.localtime())
                df.to_csv('M',"{0:.2f}".format(T),'.csv') #Este es el nombre del archivo, debería ser: M (de medio, por los valores intermedios de R), y luego la T con 2 digitos.
...
for i in range(N):
    ...
    while R<85:
        ...
        df=pd.DataFrame(mediciones)
        #print(time.localtime())
        df.to_csv('B',"{0:.2f}".format(T),'.csv') #Este es el nombre del archivo, debería ser: B (de bajo, por los valores bajos de R), y luego la T con 2 digitos.
...
im = 20 #Intervalo de medición para activar el if
